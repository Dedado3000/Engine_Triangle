...
GLuint ModuleTexture::LoadTexture(const char* image_file_name, const char* route_path, const char* route_fbx, std::string &size_data) {
	...
	App->console->AddToConsole(" ");
	App->console->AddToConsole("------CHARGING NEW TEXTURE------");

	//route_path
	char* path = new char[strlen(image_file_name) + strlen(route_path) + 1 + 1];
	strcpy(path, route_path);
	strcat(path, image_file_name);



	image = FullLoadFromRoute(path, res, size_data);

	if (FAILED(res)) {
		const char* log = "Not Found at the route from Path Route "; 
		App->console->AddToConsole(log);

		//route_fbx
		char* fbx = new char[strlen(image_file_name) + strlen(route_fbx) + 1 + 1]; 
		strcpy(fbx, route_fbx);
		strcat(fbx, image_file_name);

		image = FullLoadFromRoute(fbx, res, size_data);
		if (FAILED(res)) {
			const char* log = "Not Found at the route from PBX"; 
			App->console->AddToConsole(log);


			char* ret = new char[strlen(image_file_name) + 10 + 1 + 1]; 
			strcpy(ret, "Textures\\");
